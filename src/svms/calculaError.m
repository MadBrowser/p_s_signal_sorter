...
function error = calculaError(matriz, origen)
    if(origen == 0)
        ...
        denominador = error + matriz(1,2) + matriz(2,1);
        %+ matriz(2,3); 
        %+ matriz(1,3)
    ...
    elseif(origen >= 1)% indicadores de exactitud por clase y general
        ...
        error{1,4} = sum(diag(matriz)) / (sum(matriz(:,1)) + sum(matriz(:,2)) );
        %+ sum(matriz(:,3))
        if(origen == 2)
            error{1,5} = (matriz(1,1)/sum(matriz(1,:)) + matriz(2,2)/sum(matriz(2,:)) );
            %+ matriz(3,3)/sum(matriz(3,:))) / 3;
            P0 = sum(diag(matriz))/(sum(matriz(1,:)) + sum(matriz(2,:)) );
            %+ sum(matriz(3,:))); 
            Pe = (sum(matriz(1,:))*sum(matriz(:,1)) + sum(matriz(2,:))*sum(matriz(:,2))  + sum(matriz(2,:)) );
            %+ sum(matriz(3,:)))^2; 
            %+ (sum(matriz(3,:))*sum(matriz(:,3))))/(sum(matriz(1,:))
